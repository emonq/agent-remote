...
不用去开发者后台手动建应用——服务启动后打开引导页，飞书扫码确认即可：自动创建企业自建应用、开通权限、配置长连接事件与卡片回调、创建机器人底栏控制面板、提交应用发布，并绑定你的飞书账号。
...
裸跑同理：`npm install && npm start`，日志里会打印 `/setup?key=...` 地址。凭据存在 SQLite（`data` 卷），重启不丢。应用版本需企业审批时，审批通过后生效；多用户模式各账号再到网页生成绑定码绑定个人飞书。

服务启动时会检查自动配置指纹：首次接管已有凭据，或升级后底栏菜单、事件、回调配置发生变化时，会自动同步并提交一个新版本；配置未变化的普通重启不会重复发布。扫码创建的应用会自动获得所需配置权限。已有或手动创建的应用还需先开通 `application:application:patch` 并发布一次，只有 App ID / App Secret 不能自行获得该权限。应用正在审核时本次同步会跳过成功标记，并在下次启动重试。

> 自动配置任一步失败时，扫码页会保留已创建的应用凭据并显示具体错误；可按下方手动步骤补全配置。
...
3. **权限管理**开通：
   ...
   - `application:application:patch` — 启动时自动同步底栏菜单、事件、回调并提交版本（已有应用需手动开通并发布一次）
   - `im:chat`（可选）
4. **事件与回调 → 事件配置 → 使用长连接接收事件**，添加事件：`接收消息 im.message.receive_v1`
5. **事件与回调 → 回调配置 → 使用长连接接收回调**，添加回调：`卡片回传交互 card.action.trigger`
6. **发布新版本并通过审批**——权限变更必须走版本发布才生效，最容易漏的一步

### 飞书底栏控制面板（手动配置时可选）

机器人底栏可打开一张交互式控制卡片，直接切换 Codex / Claude Code 的 Stop 拦截和六类通知。卡片与网页共用同一份设置。

1. 进入「应用能力 → 机器人 → 自定义菜单」，添加名为「控制面板」的菜单项。
2. 响应方式选「推送事件」，事件键填 `agent_remote_settings`。
3. 在「事件与回调」中订阅「机器人自定义菜单事件」（`application.bot.menu_v6`）。
4. 发布新版本并通过审批。

扫码新建应用时，上述菜单、事件与发布步骤均会自动完成。
...
WebUI 的「Stop 拦截」可分别控制 Claude Code 和 Codex。关闭某一项后，该 agent 的任务结果仍会推送到飞书，但服务会立即放行，不再等待引用回复或「到此为止」。
